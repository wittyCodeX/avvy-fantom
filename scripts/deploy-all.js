...
async function main() {
  ...
  const rainbowTableV1 = await RainbowTableV1.deploy(
    contractRegistryV1.address
  );
  ...
  const ConstraintsV1 = await hre.ethers.getContractFactory("ConstraintsV1");
  const constraintsV1 = await ConstraintsV1.deploy(constraintsVerifier.address);

  await constraintsV1.deployed();
  console.log(`ConstraintsV1 deployed to ${constraintsV1.address}`);

  // await hre.run("verify:verify", {
  //   address: constraintsV1.address,
  //   constructorArguments: [constraintsVerifier.address]
  // });

  // 12. PricingOracleV1 Contract Deployment
  const PricingOracleV1 = await hre.ethers.getContractFactory(
    "PricingOracleV1"
  );
