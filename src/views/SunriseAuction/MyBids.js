...
class MyBids extends React.PureComponent {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.auctionPhases) {
      ...
      if (
        now >= claimStartsAt &&
        now < claimEndsAt &&
        !this.state.hasLoadedWinningBids
      ) {
        ...
        <div className="max-w-md m-auto">
          <div className="mb-8">
            <components.labels.Information
              text={"You haven't placed any bids"}
            />
          </div>
    ...
    return (
      <div className="max-w-screen-md m-auto mt-4">
        <div className="text-center font-bold text-lg">
          {'The Sunrise Auction is over'}
        </div>
        <div className="text-center">
          {'You may now register any domains that are still available.'}
        </div>
        <div className="max-w-sm m-auto mt-8">
          <components.buttons.Button
            text={'View My Domains'}
            onClick={(navigator) =>
              services.linking.navigate(navigator, 'MyDomains')
            }
          />
        </div>
  ...
  renderClaim() {
    ...
    const isSubmitted = (name) =>
      this.props.unsubmittedBidNames.indexOf(name) === -1
    ...
    const keys = Object.keys(bids)
      .sort()
      .filter((name) => isSubmitted(name))
    const nameData = this.props.nameData

    if (this.props.loadedBidProgress < 100)
      return (
    ...
    this.props.revealedBids.forEach((bid, index) => {
      ...
      if (
        this.props.winningBidsLoaded &&
        this.state.isConnected &&
        auctionResults
      ) {
        fullBidTotal = fullBidTotal.add(ethers.BigNumber.from(bid.amount))
        if (auctionResults[key] && auctionResults[key].isWinner) {
          bidTotal = bidTotal.add(
            ethers.BigNumber.from(auctionResults[key].auctionPrice),
          )
          registrationTotal = registrationTotal.add(
            ethers.BigNumber.from(nameData[key].priceUSDCents),
          )
    ...
    if (this.props.revealedBids.length === 0)
      return (
        <>
          <div className="mt-4 mb-4 text-lg text-center font-bold">
            {'Sunrise Auction - Claim'}
          </div>
          <div className="max-w-md m-auto">
            ...
            <div className="font-bold mt-4">{'What can I do now?'}</div>
            <div className="mt-4 text-gray-700 dark:text-gray-400">
              {
                'Once the auction is over, you will be able to register domains. Please return when the auction has completed.'
              }{' '}
              <Link
              ...
              >
                {
                  'Auction dates are available in the Sunrise Auction description.'
                }
              </Link>
            </div>
          </div>
    ...
    return (
      <>
        <components.Modal
          show={this.props.claimGenerateProofs.length > 0}
          ref={(ref) => {
        ...
        <div className="md:flex md:mt-2 md:mx-4">
          <div className="w-full md:mr-8">
            <div>
              <div className="mt-4 text-lg text-center font-bold md:text-left md:mt-0 md:text-xl">
                {'My Bids - Claim'}
              </div>
              <div className="text-md text-left text-gray-500 mb-4 max-w-sm m-auto md:text-left md:m-0">
                {'During this stage you must claim the auctions you have won.'}
              </div>
            ...
            {missingEnhanced > 0 ? (
              <div>
                <components.labels.Error
                  text={`You have ${missingEnhanced} Enhanced Privacy ${
                    missingEnhanced === 1 ? 'domain' : 'domains'
                  } which ${
                    missingEnhanced === 1 ? 'is' : 'are'
                  } hidden. To claim ${
                    missingEnhanced === 1 ? 'it' : 'them'
                  }, you must reveal ${missingEnhanced === 1 ? 'it' : 'them'}.`}
                />
                <div className="mt-4">
            ...
            {_keys.map((bid, index) => {
              ...
              const isOwned =
                this.props.nameData[key].status === 'REGISTERED_SELF'
              return (
                <div
                ...
                >
                  ...
                  <div className="">
                    Your Bid: {services.money.renderWFTM(bid.amount)}
                  </div>
                  {result.isWinner ? (
                    <div className="">
                      Cost to Claim:{' '}
                      {services.money.renderWFTM(
                        auctionResults[key].auctionPrice,
                      )}
                    </div>
                  ) : null}
                  <div className="">
                    Yearly registration fee:{' '}
                    {services.money.renderUSD(nameData[key].priceUSDCents)}
                  </div>
                  {this.state.isConnected ? (
                    <div className="flex mt-2">
                      <div className="mr-2">
                        {result.type === 'NO_WINNER' ? (
                          <components.labels.Error
                            text={'No participants have enough WFTM to claim'}
                            size={'xs'}
                          />
                        ) : this.props.claimedNames[key] || isOwned ? (
                          <components.labels.Success
                            text={'You have claimed this name'}
                          />
                        ) : result.type === 'IS_CLAIMED' ? (
                          <components.labels.Error
                            text={'You lost this auction'}
                          />
                        ) : result.isWinner ? (
                          <components.labels.Information
                            text={'You have won this auction'}
                          />
                        ) : (
                          <components.labels.Error
                            text={'You lost this auction'}
                  ...
                  {result.isWinner && result.type !== 'IS_CLAIMED' ? (
                    <div className="w-32 mt-4">
                      <components.buttons.Button
                        sm={true}
                        text={'Claim'}
                        onClick={() => this.props.claim(key)}
                        loading={this.props.isClaimingDomain[key]}
                      />
                    </div>
                  ) : null}
                </div>
          ...
          <div className="max-w-sm w-full m-auto mt-8 md:flex-shrink-0 md:ml-4 md:pl-4 md:mt-0">
            <div className="md:bg-gray-100 md:dark:bg-gray-800 md:rounded-lg md:p-4">
              <Summary.FullSummary
                subtitle={
                  this.props.winningBidsLoaded && this.state.isConnected
                    ? '(Totals for auctions that you won)'
                    : null
                }
                  ...
                  <div className="mt-4 max-w-sm m-auto">
                    {allClaimed ? (
                      <>
                        ...
                        <components.labels.Success
                          text={
                            'You have claimed all of the auctions you won. Congratulations!'
                          }
                        />
                        <div className="mt-4">
                          <components.buttons.Button
                      ...
                      <>
                        <div className="mb-4 text-center">
                          You have {toClaim}{' '}
                          {toClaim === 1 ? 'domain' : 'domains'} to claim.
                          {toClaim > 12
                            ? ' You can only claim 12 per transaction.'
                            : ''}
                        </div>
                        <components.buttons.Button
                          text={
                            toClaim > 12 ? 'Claim Next 12 Domains' : 'Claim All'
                          }
                          onClick={() => this.props.claimAll()}
                          loading={this.props.isClaimingDomains}
                        />
                      </>
                    ) : null}
                  </div>
      ...
      return (
        <>
          ...
          <div className="max-w-md m-auto">
            ...
            <div className="font-bold mt-4">{'No bids found.'}</div>
            <div className="mt-4 text-gray-700 dark:text-gray-400">
              {
            ...
            <div className="mt-4 text-gray-700 dark:text-gray-400">
              ...
              >
                {'you can restore it in your settings'}
              </Link>
              {'.'}
              <br />
              <br />
              {
                'If you are able to access the device you used for Bid Placement, your data may be on that device. If you are unable to locate the data to reveal your bid, your bid will be disqualified.'
              }
            </div>
            <div className="font-bold mt-4">{'What can I do now?'}</div>
            <div className="mt-4 text-gray-700 dark:text-gray-400">
        ...
        <components.Modal
          ...
          onClose={() => {
            ...
            const answer = window.confirm(
              'Closing this window will abandon your bid reveal. Are you sure you want to proceed?',
            )
            return answer
          }}
        >
        ...
        <div className="md:flex md:mt-2 md:mx-4">
          <div className="w-full md:mr-8">
            <div>
              ...
              <div className="text-md text-left text-gray-500 mb-4 max-w-sm m-auto md:text-left md:m-0">
                {
                  'During this stage you must reveal your submitted bids & authorize payment in WFTM.'
                }
              </div>
              <div className="mb-8 mt-4"></div>
            </div>
            <div className="mb-8">
              {allRevealed ? (
                <components.labels.Success
                  text={'All of your bids have been revealed.'}
                  justify="justify-flex-start"
                />
              ) : (
                <components.labels.Information
                  text={'Some of your bids are not revealed.'}
                  justify="justify-flex-start"
                />
              )}
            </div>
            {_keys.map((key, index) => {
              ...
              return (
                ...
                >
                  <div className="font-bold">{key}</div>
                  <div className="">
                    Your Bid: {services.money.renderWFTM(bids[key])}
                  </div>
                  <div className="">
                    Yearly registration fee:{' '}
                    {services.money.renderUSD(nameData[key].priceUSDCents)}
          ...
          <div className="max-w-sm w-full m-auto mt-8 md:flex-shrink-0 md:ml-4 md:pl-4 md:mt-0">
            <div className="md:bg-gray-100 md:dark:bg-gray-800 md:rounded-lg md:p-4">
              <div className="mb-8">
                <Summary.FullSummary
                  fullBidTotal={bidTotal}
                  bidTotal={bidTotal}
                  registrationTotal={registrationTotal}
                  showAvailable={allRevealed}
                />
              </div>
              {allRevealed ? (
                <div>
                ...
                <components.buttons.Button
                  text={'Reveal Bids'}
                  onClick={() => this.revealModal.toggle()}
                />
              )}
            </div>
            {this.renderDocumentation()}
  ...
  renderBidPlacement() {
    ...
    return (
      <div className="md:flex md:px-4 md:mt-2">
        <div className="w-full md:mr-8">
          <div className="mt-4 text-lg text-center font-bold md:text-left md:text-lg md:mt-0">
            {'My Bids - Bid Placement'}
          </div>
          <div className="text-md text-left text-gray-500 mb-4 max-w-sm m-auto md:text-left md:m-0">
            {
              'During this stage, you must place your sealed bids. No one will know the domain or price of your bid.'
            }
          </div>
          <div className="mb-8 mt-4">
            {allSubmitted ? (
              <components.labels.Success
                text={'All of your bids have been submitted.'}
                justify="justify-flex-start"
              />
            ) : (
              <components.labels.Information
                text={'Some of your bids are not submitted.'}
                justify="justify-flex-start"
              />
            )}
          </div>
          <components.Modal
            ...
            onClose={() => {
              ...
              const answer = window.confirm(
                'Closing this window will abandon your bid placement. Are you sure you want to proceed?',
              )
              return answer
            }}
          >
                ...
                <div className="flex mt-2">
                  ...
                  {_isSubmitted ? null : (
                    <div
                      ...
                      onClick={() => this.props.deleteBid(key)}
                    >
  ...
  renderConnect() {
    return (
      <div className="max-w-screen-md m-auto">
        ...
        <div className="max-w-md m-auto">
          <components.labels.Information
            text={'You must be connected to a wallet to view your bids'}
          />
          <div className="mt-8">
            <components.buttons.Button
  ...
  render() {
    ...
    const claimEndsAt = this.props.auctionPhases[3] * 1000
    const now = parseInt(Date.now())
    if (now >= bidPlacementStartsAt && now < bidRevealStartsAt)
      return this.renderBidPlacement()
    if (now >= bidRevealStartsAt && now < claimStartsAt)
      return this.renderBidReveal()
...
const mapDispatchToProps = (dispatch) => ({
  ...
  setClaimGenerateProofs: (names) =>
    dispatch(actions.setClaimGenerateProofs(names)),
})
